...
export default async function DashboardPage() {
  ...
  const agentStatus = {
    agent_id: "hexis-001",
    name: "Homunculus",
    status: heartbeatState.isPaused
      ? ("sleeping" as const)
      : ("active" as const),
  ...
  const goals = await fetchWithFallback(getGoals, {
    ...
    queued: [],
    issues: [],
    counts: { active: 0, queued: 0, backburner: 0, blocked: 0 },
  });

  // Fetch real heartbeat entries
  const heartbeatEntries = await fetchWithFallback(getRecentHeartbeats, []);

  // Fetch last heartbeat energy details (for energy breakdown and decision reasoning)
  const lastHeartbeatEnergy = await fetchWithFallback(getLastHeartbeatEnergy, {
    heartbeatNumber: 0,
    energyStart: 0,
    energyEnd: 0,
    totalCost: 0,
  ...
  const firstHbDate = heartbeatState.firstHeartbeat
    ? new Date(heartbeatState.firstHeartbeat)
    : null;
  const uptimeSeconds = firstHbDate
    ? Math.floor((Date.now() - firstHbDate.getTime()) / 1000)
    : 0;

  const vitality = {
    pulse_status: heartbeatState.isPaused
      ? ("sleeping" as const)
      : ("active" as const),
    ...
    api_latency_ms: 45,
    heartbeats_24h: heartbeatState.heartbeats24h,
    connection_health: {
      mcp_bridge: "connected" as const,
      postgres: "connected" as const,
      llm_api: "connected" as const,
    },
  ...
  const memoryDynamics = {
    semantic_density: [] as { name: string; value: number }[],
  };
  const ruminations: {
    id: string;
    content: string;
  ...
  const insightProgress = {
    ...
    threshold_for_consolidation: 100,
      ...
      <main className="container mx-auto px-6 py-6 space-y-6">
        <section className="space-y-4">
          ...
          <VitalityMetrics
            ...
            next_beat_due={vitality.next_heartbeat}
            api_latency_ms={vitality.api_latency_ms}
            heartbeats_24h={vitality.heartbeats_24h}
          />
        </section>
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          ...
          <div className="space-y-6 lg:col-span-2">
            <AgentStatusCard
              status={agentStatus}
              energyBreakdown={lastHeartbeatEnergy}
            />
            <DriveStatus
              {...await fetchWithFallback(getDriveStatus, {
                drives: [],
                avgDriveLevel: 0,
              })}
            />
            ...
            <RecentActions
              actions={await fetchWithFallback(getRecentActions, [])}
            />
          ...
          <div className="space-y-6">
            ...
            {/* Decision Reasoning */}
            <DecisionReasoning
              reasoning={lastHeartbeatEnergy.decisionReasoning}
              heartbeatNumber={lastHeartbeatEnergy.heartbeatNumber}
            />
          </div>
        </div>
      </main>
