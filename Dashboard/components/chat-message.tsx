...
export function ChatMessage({ role, content, timestamp, energyCost }: ChatMessageProps) {
  const isAssistant = role === "assistant"

  return (
    <div className={`flex gap-4 ${isAssistant ? "flex-row" : "flex-row-reverse"}`}>
      <Avatar className={`h-10 w-10 shrink-0 ${isAssistant ? "bg-primary/10" : "bg-accent/10"}`}>
        <AvatarFallback>
          {isAssistant ? <Brain className="h-5 w-5 text-primary" /> : <User className="h-5 w-5 text-accent" />}
        </AvatarFallback>
      </Avatar>
      <div className={`flex-1 space-y-2 ${isAssistant ? "" : "flex flex-col items-end"}`}>
        <div className={`flex items-center gap-2 ${isAssistant ? "" : "flex-row-reverse"}`}>
          <span className="text-sm font-medium text-foreground">{isAssistant ? "Hexis" : "You"}</span>
