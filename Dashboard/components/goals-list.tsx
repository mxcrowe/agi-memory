...
export function GoalsList({ goals }: GoalsListProps) {
  const getMetricsProgress = (
    ...
    if (
      goal.metrics.relationships_created !== undefined &&
      goal.metrics.target
    ) {
      const pct = Math.min(
        100,
        (goal.metrics.relationships_created / goal.metrics.target) * 100
      );
      return {
        value: pct,
        label: `${goal.metrics.relationships_created}/${goal.metrics.target} relationships`,
      };
    }
    ...
    if (goal.metrics.worldview_beliefs !== undefined && goal.metrics.target) {
      const pct = Math.min(
        100,
        (goal.metrics.worldview_beliefs / goal.metrics.target) * 100
      );
      return {
        value: pct,
        label: `${goal.metrics.worldview_beliefs}/${goal.metrics.target} beliefs`,
      };
    }
        ...
        {goals.active.length === 0 ? (
          ...
          goals.active.map((goal) => {
            ...
            return (
              <div
                key={goal.id}
                className="space-y-2 rounded-lg border border-border/50 bg-card/50 p-3"
              >
                  ...
                  <div className="space-y-1">
                    <div className="flex items-center justify-between text-xs">
                      ...
                      >
                        {isComplete
                          ? "✓ Complete"
                          : `${metricsProgress.value.toFixed(0)}%`}
                      </span>
                    </div>
                    <Progress
                      value={metricsProgress.value}
                      className={`h-1.5 ${
