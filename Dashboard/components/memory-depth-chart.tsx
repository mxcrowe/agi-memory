...
export function MemoryDepthChart({
  data,
  totalMemories,
  inactiveMemories,
}: MemoryDepthChartProps) {
    ...
    <Card className="p-4 bg-background/50 border-border/50">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h3 className="text-sm font-semibold text-muted-foreground">
          ...
          {totalMemories !== undefined && (
            <span className="font-normal ml-1">
              ({totalMemories.toLocaleString()} Total
              {inactiveMemories !== undefined && inactiveMemories > 0 && (
                <span className="text-yellow-400">
                  /{inactiveMemories} Inactive
                </span>
              )}
              )
            </span>
