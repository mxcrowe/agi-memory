...
import { Brain, Clock, Layers, ArrowUpRight, Loader2 } from "lucide-react";
...
export function RuminationTicker({
  ruminations,
  maintenanceState,
}: RuminationTickerProps) {
  ...
  const getTimeSinceLastMaintenance = () => {
    ...
    const diff =
      Date.now() - new Date(maintenanceState.lastMaintenanceAt).getTime();
        ...
        <div className="mb-4 space-y-3">
          ...
          <div className="grid grid-cols-3 gap-3 text-xs">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="font-medium text-foreground">
                {maintenanceState.intervalSeconds}s
              </span>
            </div>
            <div className="flex items-center gap-2">
              <Loader2
                className={`h-3 w-3 ${
                  (maintenanceState.pendingCalls || 0) > 0
              ...
              >
                {maintenanceState.pendingCalls || 0}
              </span>
            </div>
          </div>
          ...
          <div className="p-2 rounded-lg bg-cyan-500/5 border border-cyan-500/10">
            ...
            <div className="grid grid-cols-4 gap-1 text-xs">
              <div className="text-center p-1 rounded bg-background/50">
                ...
                <p className="text-sm font-bold text-foreground">
                  {maintenanceState.neighborhoodsRecomputed ?? 0}
                </p>
              </div>
              <div className="text-center p-1 rounded bg-background/50">
                <p className="text-muted-foreground text-[9px]">Cache</p>
                <p className="text-sm font-bold text-foreground">
                  {maintenanceState.cacheDeleted ?? 0}
                </p>
              </div>
              <div className="text-center p-1 rounded bg-background/50">
