...
export async function getHeartbeatState() {
  ...
  return {
    ...
    maintenanceIntervalSeconds: Number.parseInt(config?.maintenance_interval || "300"),
    activeGoals: Number.parseInt(goalCount?.count || "0"),
    llmModel: (typeof llmConfig?.value === 'object' ? llmConfig.value.model : llmConfig?.value) || "unknown",
    affectiveState: {
...
export async function getMaintenanceState() {
  ...
  return {
    ...
    neighborhoodsRecomputed: Number.parseInt(latestLog?.neighborhoods_recomputed || "0"),
